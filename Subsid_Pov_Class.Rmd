...
g1 <- ggplot(df, aes(x=sqrt(spm_inc))) + geom_histogram(bins = 36, fill="darkgreen") + 
  labs(title="Square Root Income Distribution", x ="Income", y = "Count in Bin")
...
g2 <- ggplot(df, aes(x=log(spm_inc+1))) + geom_histogram(bins = 36, fill="olivedrab") + 
  labs(title="Logarithmic Income Distribution", x ="Income", y = "Count in Bin")
...
g3 <- ggplot(df, aes(x=spm_inc^(1/3))) + geom_histogram(bins = 36, fill="seagreen") + 
  labs(title="Cube Root Income Distribution", x ="Income", y = "Count in Bin")
...
However, for our purposes, to reduce some skewing, we will remove all observations with incomes over $1,000,000 per year. Those individuals are earning much more than what an "average observation" would be expected to and are not as useful for our analysis.
...
We can do the same with the rest of our numeric variables. In the end, we came up with the following filtering rule to remove unnecessary high-leverage observations from our data.

```{r removeOutliers}
df <- filter(df, (spm_inc < 1000000)&(snap < 20000)&(house_sub < 30000)&
               (slunch < 10000)&(energy < 7000)&(wic < 2500)&
               (wk_xpen < 12500)&(med_xpen < 50000)&(cc_xpen < 10000)&
               (moop < 20000)&(spm_hi_prem < 30000)&(hi_prem < 30000)&
               (fed_tax<100000)&(st_tax<70000)&(eitc<75000)&(fica<40000))
